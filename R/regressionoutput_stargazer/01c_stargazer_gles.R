...
mod1 <- lm(jamaika_z ~ migration_z ,data = gles)
summary(mod1)

mod2 <- lm(jamaika_z ~ migration_z + zufr_groko_z,data = gles)
summary(mod2)
...
stargazer(mod1,mod2,  #regression models 
          ...
          column.labels = c("linkes model","rechts model"), # column labels for mod1/mod2
...
stargazer(mod1,mod2,  #regression models 
          ...
          style = "default",  # style (choice see below)
          summary = NULL,  # logical vector: output summary statistics when given data.frame
          out = "table2.html", # path and output of file
          out.header = FALSE, # logical vector: should output file contain code-header?
          ...
          covariate.labels = c("Migration",  # Covariate Labels
                               ...
                               "Intercept"),
          dep.var.caption = "Abh. Var", # Caption (Top) of dependent variable
          dep.var.labels = c("Sympathie Jamaika"), # dependent variable label
